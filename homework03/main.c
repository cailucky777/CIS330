...

{
  ...
  *csr_vals = (double*)malloc(sizeof(double)*nnz);
  ...
  //initial csr_row_ptr and compute how many nnz in per row.  
  ...
  for(unsigned int j = 0; j < nnz; j++){
      (*csr_row_ptr)[row_ind[j]-1]++;
  }

  unsigned int *res_ptr = (int *)malloc(sizeof(int)* m);

 
  //add prefix to get row_ptr using sum of the each row nnz. 
  unsigned int sumpre = 1;
  for(unsigned int i = 0; i < (m+1); i++){
      res_ptr[i] = sumpre;
      sumpre += (*csr_row_ptr)[i];   
  }
  //copy in row_ptr.
  for(unsigned int i = 0; i<(m+1); i++){
      (*csr_row_ptr)[i] = res_ptr[i];
  }
  //backup for after use.(from hint)
  unsigned int *csr_row_ptr_bk = (int *)malloc(sizeof(int)* m);
  
  for(unsigned int i = 0; i < m; i++){
       csr_row_ptr_bk[i] = (*csr_row_ptr)[i];
  }
  

  //to assign correct number of col_ind and val. 
  for(unsigned int i = 0; i < nnz; i++){
      unsigned int row_index = row_ind[i];  
      unsigned int target_index = csr_row_ptr_bk[row_index-1];
      (*csr_col_ind)[target_index] = col_ind[i]-1;
      (*csr_vals)[target_index] = val[i];
      csr_row_ptr_bk[row_index-1]++;
  }
...
{
  // get begin and end index into col and val for row.
  int i;
  int j;
  for(i=0; i<m; i++){
      for(j = csr_row_ptr[i]; j < csr_row_ptr[i+1]; j++){
          res[i] += (long double)(csr_vals[j] *(long double) vector_x[csr_col_ind[j]]);
      }
  }
}
