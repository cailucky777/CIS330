...
/* This function is to implemet spmv.

input parameters:
int*  matrix	2D matrix we wish to print2D matrix we wish to print
int   vec	shows the 2D matrix vector
int   res   the result of spmv.
int   rows	# of rows of input matrix (src)
int   cols	# of cols of input matrix (src)
return parameters:
none
...
*/
void spmv(int **matrix, int *vec, int *res, int rows, int cols)
{
              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        // HINT: spmv 
        //spmv(int **matrix, int *vec, int *res, int rows, int cols)
        int find_neighbor = 0;
        ...
        // Before we begin, eliminate vertices(to zero) that have already been visited
        for (int i = 0; i < rows; i++){
            if (color[i]){
        ...
        // the source vertex)
        //the distance from the source node to the recieving node is equal to the number of iterations.
